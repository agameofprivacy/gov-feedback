    ...
    render() {
        ...
        collections.forEach(function(collection, index){
            ...
            collection.entries.forEach(function(entry, index){
                entries.push(<span onClick={this.handleClick} value={entry.identifier} key={index} className={"pill section__collections-container__entry" + (collection.label === "上級機關" ? " pill--primary" : " pill--secondary")}>{entry.name}</span>)
            }.bind(this))
